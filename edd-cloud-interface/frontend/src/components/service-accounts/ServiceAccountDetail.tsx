...
import type { ServiceAccount, ApiToken } from "@/types";
...
interface ServiceAccountWithScopes extends Omit<ServiceAccount, "scopes"> {
  scopes: Record<string, string[]>;
}

interface ApiTokenWithTimestamps extends Omit<ApiToken, "expires_at" | "created_at" | "last_used_at"> {
  expires_at: number;
  created_at: number;
  last_used_at?: number;
}

export function ServiceAccountDetail({ id }: ServiceAccountDetailProps): React.ReactElement {
  const { userId } = useAuth();
  const navigate = useNavigate();
  const [account, setAccount] = useState<ServiceAccountWithScopes | null>(null);
  const [tokens, setTokens] = useState<ApiTokenWithTimestamps[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [editScopes, setEditScopes] = useState<Record<string, string[]> | null>(null);
  ...
  const loadAccount = async (): Promise<void> => {
    try {
      ...
      if (saRes.ok) {
        const sa: ServiceAccountWithScopes = await saRes.json();
        setAccount(sa);
        setEditScopes(null);
      }
      if (tokensRes.ok) {
  ...
  const handleSaveScopes = async (): Promise<void> => {
    ...
    try {
      ...
      if (res.ok) {
        setAccount((prev) => prev ? { ...prev, scopes: editScopes } : prev);
        setEditScopes(null);
      }
    } catch (err) {
  ...
  const handleTokenCreated = (token: string): void => {
...
function CreateTokenForm({ saId, onCreated, onCancel }: CreateTokenFormProps): React.ReactElement {
