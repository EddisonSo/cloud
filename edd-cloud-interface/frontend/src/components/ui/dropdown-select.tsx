  ...
  const dropdownContent = open
    ? createPortal(
        <div
          ref={listRef}
          style={dropdownStyle}
          // Flat panel — hairline border, bg-popover, no radius, no shadow
          className="border border-border bg-popover text-popover-foreground overflow-hidden flex flex-col"
          ...
          {searchable && (
            <div className="flex items-center gap-2 px-3 py-2 border-b border-border">
              ...
              <input
                ...
                placeholder="Search..."
                className="w-full bg-transparent text-[13.5px] outline-none placeholder:text-muted-foreground/60"
              />
            </div>
          )}
          <div className="overflow-y-auto max-h-[260px] py-1">
            {filteredGroups
              ? filteredGroups.map((group) => (
                  <div key={group.label}>
                    {/* Microlabel group header */}
                    <div className="px-3 py-1.5 font-mono text-[10px] font-medium uppercase tracking-[0.2em] text-faint">
                      {group.label}
                    </div>
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        className={cn(
          // Matches input.tsx — square, hairline, no shadow
          "flex h-9 w-full items-center justify-between border border-input bg-background px-3 py-1 text-sm transition-colors duration-150",
...
function OptionItem({
  ...
  return (
    <div
      ...
      className={cn(
        "flex items-center gap-2 px-3 h-9 text-[13.5px] cursor-pointer select-none transition-colors duration-100",
    ...
    >
      ...
      {isSelected && (
        <span className="shrink-0 text-primary text-[11px] font-mono">›</span>
      )}
    </div>
  );
}
