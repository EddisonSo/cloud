...
import type { FileEntry, UploadProgress, UploadResult } from "@/types";

// Module-level cache that persists across component mounts
const filesCache: Record<string, FileEntry[]> = {};  // { namespace: files[] }
...
export function useFiles() {
  ...
  const [uploadProgress, setUploadProgress] = useState<UploadProgress>({ bytes: 0, total: 0, active: false });
  ...
  const loadFiles = useCallback(async (namespace?: string, forceRefresh: boolean = false): Promise<FileEntry[]> => {
  ...
  const clearFilesCache = useCallback((namespace?: string) => {
    if (namespace) {
  ...
  const uploadFile = useCallback(async (namespace: string, onComplete?: () => void, { overwrite = false }: { overwrite?: boolean } = {}): Promise<UploadResult> => {
  ...
  const deleteFile = useCallback(async (file: FileEntry, namespace: string, onComplete?: () => void) => {
